...
/**
 * Class AddTitleHandler is used for adding title data.
 *
 * @internal
 */
final class AddTitleHandler extends AbstractTitleHandler implements AddTitleHandlerInterface
{
    /**
     * {@inheritdoc}
     */
    public function handle(AddTitleCommand $command): TitleId
    {
        ...
        try {
            ...
            if (false === $title->add()) {
                throw new CannotAddTitleException('Unable to add title');
            }
            ...
            throw new TitleException('An unexpected error occurred when adding contact', 0, $exception);
        }

        return new TitleId((int) $title->id);
    }
}
