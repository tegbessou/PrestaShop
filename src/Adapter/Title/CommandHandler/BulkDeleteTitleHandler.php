...
/**
 * Class BulkDeleteTitleHandler is used for bulk delete titles.
 *
 * @internal
 */
final class BulkDeleteTitleHandler extends AbstractTitleHandler implements BulkDeleteTitleHandlerInterface
{
    /**
     * {@inheritdoc}
     */
    public function handle(BulkDeleteTitleCommand $command): void
    {
        foreach ($command->getTitleIds() as $titleId) {
            ...
            $this->assertTitleWasFound($titleId, $title);

            try {
                if (false === $title->delete()) {
                    throw new CannotDeleteTitleException(sprintf('Unable to delete Title object with id "%s"', $titleId->getValue()));
                }
                $title->deleteImage();
            } catch (PrestaShopException $exception) {
                throw new TitleException(sprintf('An error occurred when deleting Title object with id "%s"', $titleId->getValue()), 0, $exception);
            }
        }
    }
}
