...
/**
 * Class GetTitleForEditingHandler is used for get title data for editing.
 *
 * @internal
 */
final class GetTitleForEditingHandler implements GetTitleForEditingHandlerInterface
{
    public function handle(GetTitleForEditing $query): EditableTitle
    {
        try {
            $title = new Gender($query->getTitleId()->getValue());

            if (0 >= $title->id) {
                throw new TitleNotFoundException(sprintf('Title object with id %s was not found', $query->getTitleId()->getValue()));
            }
            $editableTitle = new EditableTitle(
                $query->getTitleId()->getValue(),
                $title->name,
                (int) $title->type,
            );
        } catch (PrestaShopException $exception) {
            throw new TitleException(sprintf('An unexpected error occurred when retrieving title with id %s', $query->getTitleId()->getValue()), 0, $exception);
        }
